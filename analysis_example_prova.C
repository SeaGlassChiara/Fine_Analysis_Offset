/**
   root [0] .L analysis_example_prova.C
root [1] analysis_example_prova("20221015-234954", "/home/chiara/Documents/Analisi Dati Tesi/sipm4eic-analysis/decoded/decoded/20221015-234954")
...
void
analysis_example_prova(std::string runName, std::string dirname, std::vector<std::string> filenames = au::all_filenames)
{
...
//before offset
  ...
  TH2F *t_vs_phase = new TH2F("t_vs_phase", "t_vs_phase", 100, -0.6, 0.6, 2 * 60 * 25, -7 * au::coarse_to_ns, 7 * au::coarse_to_ns);
...
//after offset
  TH2F *t_vs_fine_offset = new TH2F("t_vs_fine_offset", "t_vs_fine_offset", (int)(0.25*kFineRange), 0, (int)(0.25*kFineRange), 2 * 60 * 25, -7 * au::coarse_to_ns, 7 * au::coarse_to_ns);
  TH2F *t_vs_phase_offset = new TH2F("t_vs_phase_offset", "t_vs_phase_offset", 100, -0.6, 0.6, 2 * 60 * 25, -7 * au::coarse_to_ns, 7 * au::coarse_to_ns);
    ...
    for (auto &frame_data : frames) {
      ...
      for (auto &chip_data : chips) {
        ...
        for (auto &channel_data : channels) {
          ...
          double time = hit.coarse * au::coarse_to_ns + hit.rollover * au::rollover_to_ns - calculate_calibrated_phase( hit.fine, runName, au::get_global_index( hit.fifo, hit.pixel, hit.column, hit.tdc ), 0) * au::coarse_to_ns; // [ns] 
...
//BEFORE OFFSET 
          ...
          double phase = calculate_calibrated_phase( hit.fine, runName, au::get_global_index( hit.fifo, hit.pixel, hit.column, hit.tdc ), 0);
          
          double time_raw = hit.coarse * au::coarse_to_ns + hit.rollover * au::rollover_to_ns; // [ns]
          double time = hit.coarse * au::coarse_to_ns + hit.rollover * au::rollover_to_ns - phase * au::coarse_to_ns; // [ns]
          double delta_raw = time_raw - reference;
          double delta = time - reference; 
         
          t_vs_fine->GetXaxis()->SetTitle("fine");
          t_vs_fine->GetYaxis()->SetTitle("time - reference");
          //draw t vs fine graph from which to fit and get offset parameter
           t_vs_fine->Fill(hit.fine, delta);
           
          //create fit to get offset parameter
          TF1 *fit_offset = new TF1("fit_offset", "gaus");
           
           //projection so to fit with Gaus
           TH1D *py = t_vs_fine->ProjectionY();         
           py->Fit("fit_offset", "SQ");
           
           //get offset parameter
           double offset = fit_offset->GetParameter(1);

	
	 t_vs_phase->GetXaxis()->SetTitle("phase");
         t_vs_phase->GetYaxis()->SetTitle("time - reference");
         
         traw_vs_fine->GetXaxis()->SetTitle("fine");
         traw_vs_fine->GetYaxis()->SetTitle("time_raw - reference");
         
         traw_vs_phase->GetXaxis()->SetTitle("phase");
         traw_vs_phase->GetYaxis()->SetTitle("time_raw - reference");
         
         
	  t_vs_phase->Fill(calculate_calibrated_phase( hit.fine, runName, au::get_global_index( hit.fifo, hit.pixel, hit.column, hit.tdc ), offset), delta );
	  traw_vs_fine->Fill(hit.fine, delta_raw);
  	  traw_vs_phase->Fill(calculate_calibrated_phase( hit.fine, runName, au::get_global_index( hit.fifo, hit.pixel, hit.column, hit.tdc ), offset ), delta_raw);
...
//AFTER OFFSET 
          ...
          double time_offset = hit.coarse * au::coarse_to_ns + hit.rollover * au::rollover_to_ns - calculate_calibrated_phase( hit.fine, runName, au::get_global_index( hit.fifo, hit.pixel, hit.column, hit.tdc ), offset) * au::coarse_to_ns; // [ns]
          double delta_offset = time_offset - reference; 
            
           t_vs_fine_offset->Fill(hit.fine, delta_offset);
           t_vs_phase_offset->Fill(calculate_calibrated_phase( hit.fine, runName, au::get_global_index( hit.fifo, hit.pixel, hit.column, hit.tdc ), offset), delta_offset );
  ...
  auto fout = TFile::Open("analysis_example_prova_offset.root", "RECREATE");
  hN->Write();
    ...
    traw_vs_fine->Write();
    traw_vs_phase->Write();
    t_vs_fine_offset->Write();
    t_vs_phase_offset->Write();
