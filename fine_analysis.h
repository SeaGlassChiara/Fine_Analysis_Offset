...
double  calculate_calibrated_phase      ( int kFineParameter, double kCalibrationMin, double kCalibrationMax, double offset);
double  calculate_calibrated_phase      ( int kFineParameter, global_identifier_t kFilters, double offset);
double  calculate_calibrated_phase      ( int kFineParameter, TString kRunTag, int kGlobalIndex, double offset) { return calculate_calibrated_phase(kFineParameter,{kRunTag,kGlobalIndex}, offset); };
...
( int kFineParameter, double kCalibrationMin, double kCalibrationMax, double offset) {
    ...
    kCurrentPhase = kCurrentPhase - offset;
...
double
calculate_calibrated_phase
( int kFineParameter, global_identifier_t kFilters, double offset) { return calculate_calibrated_phase(kFineParameter, get_fine_calibration_min(kFilters), get_fine_calibration_max(kFilters), offset); }
 ...
 ( std::vector<TString> kInputFileNames, TString kRunTag, TString kOutputFileName="", bool kRecalculate=false ) {
    ...
    for ( auto kCurrentFileName : kInputFileNames ) {
        ...
        for (int iEv = 0; iEv < nEvents; ++iEv) {
            kCurrentTree->GetEntry(iEv);
            int iCurrentIndex = get_global_index( kCurrentData.fifo, kCurrentData.pixel, kCurrentData.column, kCurrentData.tdc );
            hFine_All_Tuned->Fill( iCurrentIndex, calculate_calibrated_phase(kCurrentData.fine,kRunTag,iCurrentIndex, 0) );
